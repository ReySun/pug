...
var require_optional = function(name, options) {
  ...
  // Locate this module's bower.json file
  ...
  var object = JSON.parse(fs.readFileSync(f('%s/bower.json', location)));
  // Is the name defined by interal file references
  var parts = name.split(/\//);

  // Optional dependencies exist
  if(!object.peerOptionalDependencies) {
    throw new Error(f('no optional dependency [%s] defined in peerOptionalDependencies in bower.json', parts[0]));
  ...
  // Resolve the location of the module's bower.json file
  ...
  var dependentOnModule = JSON.parse(fs.readFileSync(f('%s/bower.json', location)));
  // Get the version
